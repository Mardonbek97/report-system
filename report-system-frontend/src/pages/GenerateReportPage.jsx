...
const authHeaders = () => {
  ...
  return { "Content-Type": "application/json", "Accept-Language": "UZ", ...(token ? { Authorization: `Bearer ${token}` } : {}) };
};
...
const GenerateReportPage = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        const res = await fetch("http://localhost:8080/api/reports", { headers: authHeaders() });
  ...
  return (
    <div style={s.wrapper}>
      ...
      <div style={s.tableFooter}>
        Jami: <strong>{filtered.length}</strong> ta report{search && ` (qidiruv: "${search}")`}
      </div>
    </div>
  );
};
