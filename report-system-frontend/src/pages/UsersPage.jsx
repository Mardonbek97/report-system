...
const UsersPage = () => {
  ...
  const statusColor = (status) => {
    const map = { ACTIVE: { bg: "#dcfce7", color: "#16a34a" }, INACTIVE: { bg: "#f1f5f9", color: "#64748b" }, BLOCKED: { bg: "#fef2f2", color: "#dc2626" } };
    return map[status] || { bg: "#f1f5f9", color: "#64748b" };
  };

  const roleColor = (role) => {
    const map = { ADMIN: { bg: "#ede9fe", color: "#7c3aed" }, USER: { bg: "#dbeafe", color: "#2563eb" }, MANAGER: { bg: "#fef9c3", color: "#ca8a04" } };
    return map[role] || { bg: "#f1f5f9", color: "#64748b" };
  };

  const formatDate = (dt) => {
    if (!dt) return "—";
    return new Date(dt).toLocaleDateString("uz-UZ", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false });
  };

  const openModal = (user) => { setModalUser(user); setNewPassword(""); setModalError(""); setModalSuccess(""); };
  const closeModal = () => { if (modalLoading) return; setModalUser(null); setNewPassword(""); setModalError(""); setModalSuccess(""); };
  ...
  const handleBlockUser = async () => {
    ...
    try {
      const res = await api.post("/api/admin/block", { username: blockTarget.user.username, statusEnum: blockTarget.action });
      const text = await res.text();
      if (!res.ok) throw new Error(text || "Xatolik yuz berdi");
      setUsers((prev) => prev.map((u) => u.username === blockTarget.user.username ? { ...u, status: blockTarget.action } : u));
      closeBlockModal();
  ...
  const closeAddModal = () => { if (addLoading) return; setAddModal(false); setAddForm({ username: "", mail: "", password: "" }); setAddError(""); setAddSuccess(""); };
  ...
  const handleAddUser = async () => {
    ...
    try {
      const res = await api.post("/api/admin/add", { username: addForm.username, mail: addForm.mail, password: addForm.password });
      const text = await res.text();
      if (!res.ok) throw new Error(text || "Xatolik yuz berdi");
  ...
  if (loading) return <div style={s.center}><div style={s.spinner} /><p style={{ color: "#94a3b8", marginTop: 16 }}>Yuklanmoqda...</p></div>;
  ...
  return (
    <div style={s.wrapper}>
      ...
      <div style={s.searchBar}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Username yoki email bo'yicha qidirish..." style={s.searchInput} />
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          <thead>
            <tr style={s.theadRow}>
              {["#", "Username", "Email", "Status", "Role", "Yaratilgan", "Amal"].map((h) => <th key={h} style={s.th}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={user.id} style={{ ...s.tr, cursor: "pointer" }} onClick={() => openModal(user)}
                  ...
                  <td style={s.td}><span style={{ ...s.badge, background: sc.bg, color: sc.color }}>{user.status}</span></td>
                  <td style={s.td}><span style={{ ...s.badge, background: rc.bg, color: rc.color }}>{user.role}</span></td>
      ...
      <div style={s.tableFooter}>Jami: <strong>{filtered.length}</strong> ta foydalanuvchi{search && ` (qidiruv: "${search}")`}</div>
        ...
        <div style={s.overlay} onClick={closeModal}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            <div style={s.modalHeader}>
              ...
              <div><div style={s.modalTitle}>New Password</div><div style={s.modalSub}>@{modalUser.username}</div></div>
              <button style={s.modalClose} onClick={closeModal}>✕</button>
            </div>
            <div style={s.modalBody}>
              <label style={s.label}>Yangi parol</label>
              <div style={s.inputWrap}>
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#94a3b8" style={{ flexShrink: 0 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                <input type="password" value={newPassword} onChange={(e) => { setNewPassword(e.target.value); setModalError(""); setModalSuccess(""); }} onKeyDown={(e) => e.key === "Enter" && handleUpdatePassword()} placeholder="Kamida 7 ta belgi..." style={s.modalInput} autoFocus />
              </div>
        ...
        <div style={s.overlay} onClick={closeBlockModal}>
          <div style={{ ...s.modal, width: 380 }} onClick={(e) => e.stopPropagation()}>
            <div style={s.modalHeader}>
              ...
              <div><div style={s.modalTitle}>{blockTarget.action === "BLOCKED" ? "Foydalanuvchini bloklash" : "Blokdan chiqarish"}</div><div style={s.modalSub}>@{blockTarget.user.username}</div></div>
            ...
            <div style={s.modalFooter}>
              ...
              <button disabled={blockLoading} onClick={handleBlockUser} style={{ ...s.saveBtn, background: blockTarget.action === "BLOCKED" ? "linear-gradient(135deg,#ef4444,#dc2626)" : "linear-gradient(135deg,#22c55e,#16a34a)", opacity: blockLoading ? 0.7 : 1 }}>
        ...
        <div style={s.overlay} onClick={closeAddModal}>
          <div style={{ ...s.modal, width: 460 }} onClick={(e) => e.stopPropagation()}>
            <div style={s.modalHeader}>
              ...
              <div><div style={s.modalTitle}>Yangi foydalanuvchi</div><div style={s.modalSub}>Tizimga yangi user qo'shish</div></div>
            ...
            <div style={s.modalBody}>
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={s.inputWrap}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#94a3b8" style={{ flexShrink: 0 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                  <input type="text" value={addForm.username} onChange={(e) => { setAddForm(f => ({ ...f, username: e.target.value })); setAddError(""); }} placeholder="username kiriting..." style={s.modalInput} autoFocus />
                </div>
              </div>
              <div style={{ marginBottom: 16 }}>
                <label style={s.label}>Email</label>
                <div style={s.inputWrap}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#94a3b8" style={{ flexShrink: 0 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  <input type="email" value={addForm.mail} onChange={(e) => { setAddForm(f => ({ ...f, mail: e.target.value })); setAddError(""); }} placeholder="email@example.com" style={s.modalInput} />
                </div>
              </div>
              <div style={{ marginBottom: 4 }}>
                ...
                <div style={s.inputWrap}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#94a3b8" style={{ flexShrink: 0 }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                  <input type="password" value={addForm.password} onChange={(e) => { setAddForm(f => ({ ...f, password: e.target.value })); setAddError(""); }} onKeyDown={(e) => e.key === "Enter" && handleAddUser()} placeholder="Kamida 7 ta belgi..." style={s.modalInput} />
                </div>
              </div>
              <div style={s.modalHint}>Parol kamida <strong>7 ta belgi</strong>dan iborat bo'lishi kerak</div>
...
const s = {
  ...
  statCard: { background: "#fff", borderRadius: 10, padding: "10px 16px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", flex: 1, minWidth: 80 },
...
if (!document.getElementById("users-styles")) { styleTag.id = "users-styles"; document.head.appendChild(styleTag); }
