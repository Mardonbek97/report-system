create or replace package Rep_Core_Util is
  ...
  Procedure Import_Excel(p_rep_id    RAW,
                         p_upload_id RAW,
                         p_user      varchar2,
                         o_Errmsg    out varchar2);
...
CREATE OR REPLACE Package Body Rep_Core_Util is
  ...
  Begin
    update rep_core_log t
       set t.status = '2', t.percentage = '100', t.end_time = sysdate
     where t.id = i_rep_id
  ...
  Begin
    ...
    for i in 1 .. v_keys.count loop
      ...
      if lower(v_param_data_type_name) = 'date' then
        v_param_value := to_char(to_date(v_json.get_string(v_param_name),
                                         'dd.mm.yyyy'),
                                 'yyyymmdd');
  ...
  Begin
    ...
    Execute immediate v_sql;
    rep_core_util.Report_Log_End(p_repid => p_rep_id, i_rep_id => v_rep_id);
